...
<div class="container">
    ...
    <?php if (isset($_SESSION['notify-success'])) : ?>
        <div class="alert alert-success mt-3"><?php echo $_SESSION['notify-success']; session_unset() ?></div>
    <?php endif ?>

    <?php if (isset($_SESSION['notify-fail mt-3'])) : ?>
        <div class="alert alert-success"><?= $_SESSION['notify-fail'] ; session_unset()?></div>
    <?php endif ?>
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex align-items-center justify-content-center w-fit">
                            <form onsubmit="return confirm('Xác nhận xóa ?')"; action="destroy.php" method="post" class="me-2 form-delete-menu">
                                <button  class="btn btn-danger btn-delete-menu" type="submit" name="id" value="<?= $menu['id'] ?>">Delete</button>
                            </form>
    ...
    <div class="d-flex align-items-center justify-content-center w-100">
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <?php if ($page > 1) :?>
                    <li class="page-item">
                        <a class="page-link" href="find.php?page=<?=($page - 1)?>&&key=<?= $key ?>" aria-label="Previous">
                ...
                <?php for($i = 1 ; $i <= $endPage ; $i++): ?>
                    <li class="page-item <?= ($page == $i) ? "active" : ""?>">
                        <a class="page-link" href="find.php?page=<?=$i?>&&key=<?= $key ?>"><?= $i ?></a>
                    </li>
                <?php endfor ?>
                <?php if ($page < $endPage) : ?>
                    <li class="page-item">
                        <a class="page-link" href="find.php?page=<?=($page + 1)?>&&key=<?= $key ?>" aria-label="Next">
