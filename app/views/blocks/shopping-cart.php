                ...
                <table class="table w-100 shopping-cart-table">
                    ...
                    <tbody>
                        <c:forEach items="${cart}" var="value">
                            <tr>
                                <td>${value.getProductName()}</th>
                                <td><img src="images/${value.getProductImage()}" style="max-width: 120px; border-radius: 10px" alt="food image" /></td>
                                <td>${value.getAmount()}</td>
                                <td>${value.getTotalPrice()}</td>
                                <td>
                                    <a class="h3" href="./delete-shopping-cart?id=${value.getProductId()}"><ion-icon name="close-circle-outline"></ion-icon></a>
                                </td>                                    
                            </tr>
                        </c:forEach>
                    </tbody>
